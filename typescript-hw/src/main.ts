...
export class Card {
    ...
    AddTransaction(amountOrTransaction: number | Transaction, currency?: CurrencyEnum): string {
    ...
    GetBalance(currency: CurrencyEnum): number {
        return this.Transactions.filter(t => t.Currency === currency).reduce((sum, t) => sum + t.Amount, 0);
    }
}